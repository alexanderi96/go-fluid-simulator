...
// 	sliderLength = float64(sideMenuContentWidth) - 2*10 // or whatever value you have for 'border'

// 	rl.DrawRectangle(s.Config.WindowWidth-sideMenuTotalWidth, 0, s.Config.WindowWidth, s.Config.WindowHeight, rl.DarkGray)
...
// 	frametime := fmt.Sprintf("Frametime: %f", s.Metrics.RealFrametime)
...
// 	s.Config.UnitNumber = int32(gui.Slider(rl.Rectangle{X: float32(xContentStart), Y: float32(yContentStartTop), Width: float32(sliderLength), Height: float32(sliderThickness)}, "", "", float32(s.Config.UnitNumber), 1, 1000))
// 	yContentStartTop += 20 + 5
...
// 			if radMin, err := utils.CheckTextFloat64(radMinText); err == nil {
...
// 			if radMax, err := utils.CheckTextFloat64(radMaxText); err == nil {
...
// 			if elasticityMin, err := utils.CheckTextFloat64(elasticityMinText); err == nil {
...
// 			if elasticityMax, err := utils.CheckTextFloat64(elasticityMaxText); err == nil {
...
// 			if MassMultiplierMin, err := utils.CheckTextFloat64(massMinText); err == nil {
...
// 			if MassMultiplierMax, err := utils.CheckTextFloat64(massMaxText); err == nil {
