...
func handleCollision(uA, uB *Unit, distance float64) {
	...
	rVelNormal := vRel.Dot(impulseDirection)
	impulseMag := -(1 + e) * rVelNormal / (1/uA.Mass + 1/uB.Mass)
	...
	heatTransfer := rVelNormal * distance * 0.5
	uA.Heat += heatTransfer * (1.0 - uA.Elasticity)
	uB.Heat += heatTransfer * (1.0 - uB.Elasticity)
}
