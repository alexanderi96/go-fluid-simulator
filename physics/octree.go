...
func getOctreeWireframe(min, max math32.Vector3) *graphic.Lines {
...
func NewOctree(level int8, bounds *math32.Box3, scene *core.Node) *Octree {
...
func (ot *Octree) Clear(scene *core.Node) {
	...
	ot.TotalMass = 0
	ot.CenterOfMass = *math32.NewVec3()
	scene.Remove(ot.wf)
...
func (ot *Octree) Split(scene *core.Node) {
	...
	swb := &math32.Box3{Min: *math32.NewVector3(minX, minY, minZ), Max: *math32.NewVector3(minX+subWidth, minY+subHeight, minZ+subDepth)}
	ot.Children[0] = NewOctree(level, swb, scene)
	// children[1] = inBottom && inRight && inBack
	seb := &math32.Box3{Min: *math32.NewVector3(minX+subWidth, minY, minZ), Max: *math32.NewVector3(minX+2*subWidth, minY+subHeight, minZ+subDepth)}
	ot.Children[1] = NewOctree(level, seb, scene)
	// children[2] = inTop && inLeft && inBack
	nwb := &math32.Box3{Min: *math32.NewVector3(minX, minY+subHeight, minZ), Max: *math32.NewVector3(minX+subWidth, minY+2*subHeight, minZ+subDepth)}
	ot.Children[2] = NewOctree(level, nwb, scene)
	// children[3] = inTop && inRight && inBack
	neb := &math32.Box3{Min: *math32.NewVector3(minX+subWidth, minY+subHeight, minZ), Max: *math32.NewVector3(minX+2*subWidth, minY+2*subHeight, minZ+subDepth)}
	ot.Children[3] = NewOctree(level, neb, scene)
	// children[4] = inBottom && inLeft && inFront
	swf := &math32.Box3{Min: *math32.NewVector3(minX, minY, minZ+subDepth), Max: *math32.NewVector3(minX+subWidth, minY+subHeight, minZ+2*subDepth)}
	ot.Children[4] = NewOctree(level, swf, scene)
	// children[5] = inBottom && inRight && inFront
	sef := &math32.Box3{Min: *math32.NewVector3(minX+subWidth, minY, minZ+subDepth), Max: *math32.NewVector3(minX+2*subWidth, minY+subHeight, minZ+2*subDepth)}
	ot.Children[5] = NewOctree(level, sef, scene)
	// children[6] = inTop && inLeft && inFront
	nwf := &math32.Box3{Min: *math32.NewVector3(minX, minY+subHeight, minZ+subDepth), Max: *math32.NewVector3(minX+subWidth, minY+2*subHeight, minZ+2*subDepth)}
	ot.Children[6] = NewOctree(level, nwf, scene)
	// children[7] = inTop && inRight && inFront
	nef := &math32.Box3{Min: *math32.NewVector3(minX+subWidth, minY+subHeight, minZ+subDepth), Max: *math32.NewVector3(minX+2*subWidth, minY+2*subHeight, minZ+2*subDepth)}
...
func (ot *Octree) Insert(obj *Unit, scene *core.Node) {
	...
	if oldTotalMass == 0 {
		ot.CenterOfMass = obj.Mesh.Position()
	} else {
		meshPos := obj.Mesh.Position()
		ot.CenterOfMass.AddScalar(oldTotalMass).Add(meshPos.AddScalar(obj.Mass)).AddScalar(1 / ot.TotalMass)
	}
...
func (ot *Octree) insertUnitIntoChildren(obj *Unit, scene *core.Node) {
	...
	if !inserted {
		...
		ot.TotalMass += obj.Mass
		meshPos := obj.Mesh.Position()
		ot.CenterOfMass.AddScalar(ot.TotalMass - obj.Mass).Add(meshPos.AddScalar(obj.Mass)).AddScalar(1 / ot.TotalMass)
	}
}
...
func (ot *Octree) getIndices(obj Unit) []int {
	...
	minX := obj.Mesh.Position().X + obj.Radius
	maxX := obj.Mesh.Position().X - obj.Radius
	minY := obj.Mesh.Position().Y + obj.Radius
	maxY := obj.Mesh.Position().Y - obj.Radius
	minZ := obj.Mesh.Position().Z + obj.Radius
	maxZ := obj.Mesh.Position().Z - obj.Radius
