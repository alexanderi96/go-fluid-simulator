...
func (s *SpaceShip) LoadShip() {
	...
	shipMat := material.NewStandard(&math32.Color{R: 0.8, G: 0.8, B: 0.9})
	shipMat.SetShininess(30)
	shipMat.SetSpecularColor(&math32.Color{R: 1, G: 1, B: 1})
	shipMat.SetOpacity(1.0)

	// Apply material to all meshes in the group
	for _, node := range group.Children() {
		if mesh, ok := node.(*graphic.Mesh); ok {
			mesh.SetMaterial(shipMat)
		}
	}

	// Add a point light to enhance illumination
	light := light.NewPoint(&math32.Color{R: 0.8, G: 0.8, B: 0.9}, 1.0)
